-- parse_item_bases.lua  <input.lua>  <output.json>
--
-- PoB2's Data/Bases/*.lua expects `local itemBases = ...` and then
-- populates that table. We dofile() with itemBases preset and dump.
...
if not input_path then
    io.stderr:write("usage: parse_item_bases.lua <input.lua> [output.json]\n")
    os.exit(2)
end
local chunk, err = loadfile(input_path)
if not chunk then io.stderr:write("loadfile failed: " .. tostring(err) .. "\n"); os.exit(1) end
local itemBases = {}
local ok, run_err = pcall(chunk, itemBases)
if not ok then io.stderr:write("execution failed: " .. tostring(run_err) .. "\n"); os.exit(1) end

local json_str = to_json(itemBases)
if out_path then
...
local count = 0; for _ in pairs(itemBases) do count = count + 1 end
io.stderr:write(string.format("Parsed %d bases from %s\n", count, input_path))
