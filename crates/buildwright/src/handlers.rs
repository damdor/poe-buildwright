...
pub fn poe1_tree(ctx: &Ctx, args: &[String]) -> Result<(), String> {
    ...
    // Ascendancy pocket anchors. The web JSON keeps asc groups at the
    // periphery, but the GAME shows each class's ascendancy circle in
    // the empty pocket beside that class's start. Those pockets are
    // derivable: they're the only spots near the wheel where a
    // backdrop-sized circle fits with no main-tree nodes inside. We
    // grid-scan for clearance maxima, then uniquely assign each class
    // its nearest pocket (min total distance over all assignments).
    {
        let main_pts: Vec<(f64, f64)> = {
            let mut v = Vec::new();
            for n in nodes_obj.values() {
                if !s(n.get("ascendancyName")).is_empty() {
                    continue;
                }
                let Some(gid) = n.get("group").and_then(json::Value::as_i64) else { continue };
                let g = groups.get(&gid.to_string());
                ...
                let slots = per_orbit.get(orbit).copied().unwrap_or(1.0).max(1.0);
                let angle = std::f64::consts::TAU * oidx / slots;
                v.push((gx + r * angle.sin(), gy - r * angle.cos()));
            }
            v
        };
        let clearance = |px: f64, py: f64| -> f64 {
            main_pts
                .iter()
                .map(|(x, y)| ((px - x).powi(2) + (py - y).powi(2)).sqrt())
                .fold(f64::INFINITY, f64::min)
        };
        // Candidate pockets: clearance > 650 in the wheel region,
        // greedily deduped so distinct pockets stay 1400+ apart.
        ...
        while gy <= 4500 {
            ...
            while gx <= 4500 {
                ...
                let d0 = (fx * fx + fy * fy).sqrt();
                if (800.0..=4800.0).contains(&d0) {
                    let c = clearance(fx, fy);
                    if c > 650.0 {
                        cands.push((c, fx, fy));
                    }
                }
        ...
        let mut pockets: Vec<(f64, f64)> = Vec::new();
        for &(_, x, y) in &cands {
            if pockets.iter().all(|&(px, py)| ((x - px).powi(2) + (y - py).powi(2)).sqrt() > 1400.0) {
                pockets.push((x, y));
            }
        }
        // Class starts (main-tree class_start nodes).
        let mut class_starts: Vec<(String, f64, f64)> = Vec::new();
        for n in nodes_obj.values() {
            if let Some(ci) = n.get("classStartIndex").and_then(json::Value::as_i64)
                && let Some(name) = class_names.get(ci as usize) {
                    let Some(gid) = n.get("group").and_then(json::Value::as_i64) else { continue };
                    let g = groups.get(&gid.to_string());
                    class_starts.push((
                        name.clone(),
                        f(g.and_then(|g| g.get("x"))),
                        f(g.and_then(|g| g.get("y"))),
                    ));
                }
        }
        class_starts.sort_by(|a, b| a.0.cmp(&b.0));
        // Exhaustive min-total-distance assignment (≤10 pockets, 7
        // classes — small). Deterministic given the data.
        fn assign(
            idx: usize,
            used: &mut Vec<bool>,
            classes: &[(String, f64, f64)],
            pockets: &[(f64, f64)],
            cur: f64,
            cur_pick: &mut Vec<usize>,
            ...
            if idx == classes.len() {
                *best = (cur, cur_pick.clone());
                return;
            }
            let (_, sx, sy) = &classes[idx];
            for (p, &(px, py)) in pockets.iter().enumerate() {
                if used[p] {
                    continue;
                }
                used[p] = true;
                ...
                assign(idx + 1, used, classes, pockets, cur + d, cur_pick, best);
                cur_pick.pop();
                used[p] = false;
            }
        }
        if pockets.len() >= class_starts.len() {
            let mut used = vec![false; pockets.len()];
            let mut best = (f64::INFINITY, Vec::new());
            let mut pick = Vec::new();
            assign(0, &mut used, &class_starts, &pockets, 0.0, &mut pick, &mut best);
            for (ci, &p) in best.1.iter().enumerate() {
                let (name, _, _) = &class_starts[ci];
                let (px, py) = pockets[p];
                meta.push_str(&format!("asc_anchor\t{name}\t{px}\t{py}\n"));
            }
        } else {
            ui::warn(ctx.style, "not enough asc pockets found — anchors skipped");
        }
    }
