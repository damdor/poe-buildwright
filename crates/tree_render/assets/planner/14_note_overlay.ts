...
//   * Toggle button (chevron) pins the state — saved in localStorage.
...
//   * State key is shared across builds (sidebar-collapsed is a
//     personal preference, not per-build).
// -------------------------------------------------------------------
{
  const panel = document.getElementById('panel');
  const toggle = document.getElementById('panel-toggle');
  const row = document.querySelector('.planner-row');
  const KEY = 'poe2-planner:sidebar-collapsed';
  if (panel && toggle && row) {
